...
def load_vgg(sess, vgg_path):
    ...
    return img_in, keep_prob, layer3_out, layer4_out, layer7_out
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
	...
	# Up Sampling with Order 2
    up_samp_32x = tf.layers.conv2d_transpose(conv7, num_classes, 4, strides=(2, 2), padding='same',
                                             kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

	# First Skip Connection
    skip_add_1 = tf.add(up_samp_32x, conv4)

	# Up Sampling with Order 2
    up_samp_16x = tf.layers.conv2d_transpose(skip_add_1, num_classes, 4, strides=(2, 2), padding='same',
                                             kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

	# Second Skip Connection
    skip_add_2 = tf.add(up_samp_16x, conv3)

	# Up Sampling with Order 8
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    with tf.name_scope("training"):
        ...
        train_op = optimizer.minimize(entropy_loss, global_step=glob_step)

    return logit, train_op, entropy_loss
    ...
    for epoch in range(epochs):
        for i, (img, label) in enumerate(get_batches_fn(batch_size)):
            _, loss = sess.run([train_op, cross_entropy_loss],
                                feed_dict={input_image:img, correct_label:label, keep_prob:0.4})
